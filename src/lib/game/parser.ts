...
export function parseQuestion(input: string) {
  ...
  return { parts, nPlayers: sortedPlayers.length, players: sortedPlayers, playersSet: players } as ParsedQuestion
}
...
export function displayQuestion(challenges: string[], players: string[]) {
  ...
  for (const challenge of challenges) {
    ...
    if (parsed.players.length > mostPlayers.length) mostPlayers = parsed.players;
  }
